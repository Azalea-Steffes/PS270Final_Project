...
ba_enroll_chrt|>
knitr::kable(caption= "Primary school enrollment Before and after regime change", digits=2, col.names = c("Before", "After"))
```
...
```{r}
 pri_enroll_chrt <- Data |> 
  group_by(rgjtype)|>
  summarise(mean_enroll = mean(school_enrol_primary_pct, na.rm= TRUE)) |>
  filter(rgjtype %in% c("D", "A")) |>
  pivot_wider(
    names_from= "rgjtype",
    values_from = "mean_enroll"
  ) 
pri_enroll_chrt|>
knitr::kable(caption = "primary school enrollment by regime type", digits=2, col.names = c("Authoritarian", "Democratic"))
```
